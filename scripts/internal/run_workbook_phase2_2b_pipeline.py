#!/usr/bin/env python3
"""Resumable server pipeline for newly compiled Phase 2.2B strategies only."""
...
def main() -> int:
    ...
    parser.add_argument("--backtest-root", type=Path, default=ROOT / "outputs/batches/workbook_strategies_phase2_2b")
    parser.add_argument("--deliverable-root", type=Path, default=ROOT / "outputs/deliverables/workbook_strategies_phase2_2b")
    parser.add_argument("--status", type=Path, default=ROOT / "outputs/internal_audit/strategy_workbook/phase2_2b_pipeline_status.json")
    args = parser.parse_args()
    python = sys.executable
    plan_path = ROOT / "configs/semantic_contracts/workbook_phase2_2b_strategies.json"
    strategies = sorted(json.loads(plan_path.read_text(encoding="utf-8")))
    try:
        run("compile", [python, "scripts/internal/compile_phase2_2b_strategies.py"], args.status)
        run("package_check", [python, "scripts/internal/generate_workbook_strategy_packages.py", "--check"], args.status)
        run("audit", [python, "scripts/internal/audit_strategy_workbook.py", str(args.workbook), "--output-dir", str(args.audit_root)], args.status)
        test_groups = [
            ("semantic_contract_tests", "tests/unit_tests/strategy_framework", "tests/unit_tests/strategy_framework/test_semantic_contracts.py"),
            ...
            ("lag_execution_tests", "tests/unit_tests/scripts", "tests/unit_tests/scripts/test_timeframe_lag_execution.py"),
        ]
        for stage, confcut, test in test_groups:
            run(stage, [python, "-m", "pytest", "-q", f"--confcutdir={confcut}", test], args.status)
        backtest = [
        ...
        run("final_audit", [python, "scripts/internal/finalize_phase2_2b_audit.py",
                             "--audit-root", str(args.audit_root),
                             "--backtest-root", str(args.backtest_root)], args.status)
