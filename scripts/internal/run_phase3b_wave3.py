#!/usr/bin/env python3
# ruff: noqa: E402,I001
"""Run leakage-safe Phase 3B Wave 3 coupled-parameter searches."""
...
from scripts.internal.preflight_phase3b_wave3 import WAVE1_SUMMARY
from scripts.internal.preflight_phase3b_wave3 import atomic_csv
from scripts.internal.preflight_phase3b_wave3 import atomic_json
...
from scripts.internal.preflight_phase3b_wave3 import read_csv
...
DEFAULT_OUTPUT = ROOT / "outputs/parameter_search/phase3b_wave3"
...
def load_wave3() -> tuple[list[dict[str, str]], dict[str, Any], dict[str, Any]]:
    specs = wave3_specs(read_csv(MANIFEST))
...
def wave3_code_hash() -> str:
    digest = hashlib.sha256(wave1.code_hash().encode())
    for path in (
        ROOT / "scripts/internal/run_phase3b_wave3.py",
        ROOT / "scripts/internal/preflight_phase3b_wave3.py",
        MANIFEST,
        AMENDMENT,
    ):
...
def preflight(
    ...
    specs, folds, _ = load_wave3()
    wave1_summary = json.loads(WAVE1_SUMMARY.read_text(encoding="utf-8"))
    ...
    checks = {
        "wave1_release_decision": wave1_summary.get("release_decision") == "WAVE3_READY",
        "protocol_version_fixed": wave1_summary.get("protocol_version") == PROTOCOL_VERSION,
        "wave3_spec_count": len(specs) == 35,
        "wave3_candidate_count": valid == 702,
        "fold_count": len(folds["folds"]) == 7,
        "logical_evaluation_count": logical == LOGICAL_PLAN,
        "all_baselines_present": baselines == 35,
        ...
        == json.loads(AMENDMENT.read_text(encoding="utf-8"))["parent_manifest_sha256"],
    ...
    atomic_csv(output_root / "phase3b_wave3_equivalence_manifest.csv", equivalence)
    report = {
        "status": "PASSED" if not failed else "FAILED",
        "release_decision": "WAVE3_EXECUTION_AUTHORIZED" if not failed else "WAVE3_BLOCKED",
        "protocol_version": PROTOCOL_VERSION,
        "manifest_amendment_version": json.loads(AMENDMENT.read_text())["amendment_version"],
        "checks": checks,
        "errors": failed,
        "wave3_specs": len(specs),
    ...
    if failed:
        raise RuntimeError(f"Wave 3 preflight failed: {failed}")
    return report
...
def make_oos_figure(
    ...
    fig.suptitle(f"{spec['strategy_id']} — Phase 3B Wave 3 OOS — 1m lag1m Premium Included")
    fig.tight_layout()
...
def finalize(
    ...
    equivalence = read_csv(output_root / "phase3b_wave3_equivalence_manifest.csv")
    ...
    atomic_csv(output_root / "phase3b_wave3_run_manifest.csv", all_manifest)
    atomic_csv(output_root / "phase3b_wave3_candidate_metrics.csv", all_candidates)
    atomic_csv(output_root / "phase3b_wave3_fold_selections.csv", all_selections)
    atomic_csv(output_root / "phase3b_wave3_fold_test_metrics.csv", all_tests)
    atomic_csv(output_root / "phase3b_wave3_oos_summary.csv", oos_rows)
    atomic_csv(output_root / "phase3b_wave3_baseline_comparison.csv", oos_rows)
    atomic_csv(output_root / "phase3b_wave3_parameter_stability.csv", stability_rows)
    atomic_csv(output_root / "phase3b_wave3_boundary_drift.csv", boundary_rows)
    atomic_csv(output_root / "phase3b_wave3_fold_consistency.csv", consistency_rows)
    ...
    integrity_after = wave1.integrity_snapshot(strategy_root)
    ...
    summary = {
        ...
        "wave3_specs_terminal": len(oos_rows),
    ...
    summary["release_decision"] = "WAVE5_READY" if summary["status"] == "PASSED" else "WAVE5_BLOCKED"
    atomic_json(output_root / "phase3b_wave3_validation_summary.json", summary)
...
def main() -> int:
    ...
    if integrity_path.is_file():
        ...
        integrity_before = wave1.integrity_snapshot(args.strategy_root)
    ...
    progress = {"status": "RUNNING", "planned_specs": 35, "completed_specs": 0, "failures": []}
