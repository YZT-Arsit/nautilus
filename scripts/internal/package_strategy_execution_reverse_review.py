#!/usr/bin/env python3
"""Hash-manifest and ZIP the validated strategy-first delivery."""
...
DELIVERY = ROOT / "outputs/deliverables/strategy_execution_reverse_review"
...
def main() -> None:
    ...
    with zipfile.ZipFile(archive) as zipped:
        ...
        if failed is not None:
            raise ValueError(f"ZIP integrity failed at {failed}")

    summary = {
        "status": "PASSED",
        "files": sum(path.is_file() for path in delivery.rglob("*")),
        "zip": str(archive),
        "zip_sha256": sha256(archive),
        "zip_bytes": archive.stat().st_size,
    }
    package_path = delivery.parent / "strategy_execution_reverse_review_package.json"
    package_path.write_text(json.dumps(summary, indent=2) + "\n")
