#!/usr/bin/env python3
# ruff: noqa: E402,I001
"""Validate the immutable Phase 3B Wave 3 plan without running backtests."""
...
MANIFEST = AUDIT / "phase3b_wave3_parameter_search_manifest.csv"
FOLDS = AUDIT / "phase3a_walk_forward_protocol.json"
WAVE1_SUMMARY = ROOT / "outputs/deliverables/phase3b_wave1/phase3b_wave1_validation_summary.json"
...
def wave3_specs(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    return [
        row
        for row in rows
        if row["status"] == "READY" and len(json.loads(row["searchable_parameters"])) == 2
    ]
...
def equivalence_rows(specs: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for index, members in enumerate(sorted(grouped.values(), key=lambda value: value[0]["search_id"]), 1):
        group_id = f"wave3_equivalence_{index:03d}"
            ...
            records.append(
                {
                    ...
                    **item,
                    ...
                    "reason": (
                        ...
                        if equivalent
                        else "no equivalent Wave 3 spec"
                    ),
                }
            )
...
def main() -> int:
    ...
    specs = wave3_specs(all_rows)
    folds = json.loads(FOLDS.read_text(encoding="utf-8"))["folds"]
    wave1 = json.loads(WAVE1_SUMMARY.read_text(encoding="utf-8"))
    ...
    atomic_csv(args.output_root / "phase3b_wave3_equivalence_manifest.csv", equivalence)
    checks = {
        "wave1_release_decision": wave1.get("release_decision") == "WAVE3_READY",
        "protocol_version_fixed": wave1.get("protocol_version") == PROTOCOL_VERSION,
        "wave3_spec_count": len(specs) == 35,
        "wave3_candidate_count": valid_total == 702,
        "fold_count": len(folds) == 7,
        "logical_evaluation_count": 2 * valid_total * len(folds) + 2 * len(specs) * len(folds) == 10318,
    ...
    report = {
        ...
        "release_decision": "WAVE3_EXECUTION_AUTHORIZED" if not errors else "WAVE3_BLOCKED",
