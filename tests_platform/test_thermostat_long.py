"""Focused tests for the Thermostat long engine (regime-switching, mirror of short).
...
switch (20) to drive a swing-entered long into the trend regime and out via the
ATR protective stop. Runnable via ``pytest tests_platform -k thermostat``.
"""
...
from strategies.thermostat_long.config import ThermostatLongConfig as Cfg
from strategies.thermostat_long.engine import ThermostatLongEngine as Engine
...
def test_swing_regime_entry_and_exit():
    # swing_trend_switch=100 -> CMI always below it -> always the swing regime.
    reasons, _ = _reasons(_chop(80), swing_trend_switch=100.0)
    assert reasons["swing_enter_long"] >= 1
    assert reasons["swing_exit"] >= 1
    # no trend-regime branches should fire when the regime is pinned to swing.
    assert reasons["trend_enter_long"] == 0
    assert reasons["trend_exit"] == 0


# -- trend regime (Bollinger breakout) --------------------------------------

def test_trend_regime_entry_and_exit():
    # swing_trend_switch=0 -> CMI always >= it -> always the trend regime.
    bars = _noise(55)
    bars += [(100 + i * 2.5, 100 + i * 2.5 + 1.0, 100 + i * 2.5 - 0.4, 100 + i * 2.5 + 0.9)
             for i in range(16)]                                  # break the upper band
    bars += [(140 - i * 5, 140 - i * 5 + 0.5, 140 - i * 5 - 1.5, 140 - i * 5 - 1.3)
             for i in range(25)]                                  # drop to the MA / lower band
    reasons, _ = _reasons(bars, swing_trend_switch=0.0)
    assert reasons["trend_enter_long"] >= 1
    assert reasons["trend_exit"] >= 1
    assert reasons["swing_enter_long"] == 0


# -- swing position exits via the ATR protective stop in the trend regime ----

def test_swing_position_atr_protective_stop_in_trend():
    # Default switch (20): a swing long taken as the advance begins is carried
    # into the trend regime, then stopped out on the drop by Low <= Entry-3*ATR.
    # The advance has no lower wick (low == open) so swing_exit never triggers
    # first, keeping the swing position alive into the trend regime.
    bars = _chop(60)
    bars += [(100 + i * 2, 100 + i * 2 + 2.0, 100 + i * 2, 100 + i * 2 + 1.8)
             for i in range(25)]                                  # clean advance (CMI climbs to trend)
    bars += [(148 - i * 3, 148 - i * 3 + 0.5, 148 - i * 3 - 1.0, 148 - i * 3 - 0.8)
             for i in range(30)]                                  # crash trips the ATR stop
    reasons, _ = _reasons(bars)
    assert reasons["swing_enter_long"] >= 1
    assert reasons["swing_prot_stop_exit"] >= 1
