...
CONTRACTS: tuple[SemanticContract, ...] = (
    ...
    SemanticContract("MODELLED_BOUNDED_EQUAL_LADDER", 1,
                     "explicit finite grid/ladder/pyramid uses source triggers, four equal layers when omitted, and a 1x exposure cap",
                     ("default_layers", "max_abs_exposure"),
                     (("default_layers", 4), ("max_abs_exposure", 1.0)),
                     SemanticProvenance.MODELLED_BASELINE_INTERPRETATION),
    SemanticContract("MODELLED_STANDARD_REGULAR_DIVERGENCE", 1,
                     "regular divergence at causal 2-left/2-right price pivots using the named indicator at the same timestamp within 60 completed bars",
                     ("side_bars", "lookback"), (("side_bars", 2), ("lookback", 60)),
                     SemanticProvenance.MODELLED_BASELINE_INTERPRETATION),
