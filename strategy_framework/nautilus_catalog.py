...
**分层边界**：这是 Nautilus 集成层（``strategy_framework``）里的一个 *可选* 数据接入
桥。它**刻意**不放在 ``data_engine`` 中——``data_engine`` / ``feature_engine`` 两层
对 Nautilus **零依赖、零 import**，可以整包拿去接任何框架。本桥的依赖方向是
``strategy_framework`` → ``data_engine.transforms``（允许的方向），把 Nautilus 的
``QuoteTick`` 归一化成 data_engine 的中性 tick（dict），再交给
:func:`data_engine.transforms.aggregate_ticks_to_bars` 聚合成标准分钟线。**不**把
Nautilus 对象泄漏进 data_engine / feature_engine。

没装 nautilus_trader 时，本模块导入仍然成功（顶层不 import nautilus），只有真正
调用时才懒加载，并在缺失时给出清晰错误。
"""
