"""MA-Crossover Channel-Breakout short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`MaCrossoverChannelShortEngine`. Reads raw open/high/low/close/volume off
...
from strategies.ma_crossover_channel_short.engine import HOLD, MaCrossoverChannelShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "machan_s_bar_open"
_HIGH = "machan_s_bar_high"
_LOW = "machan_s_bar_low"
_CLOSE = "machan_s_bar_close"
_VOLUME = "machan_s_bar_volume"


class MaCrossoverChannelShortStrategy:
    """Adapter: drive :class:`MaCrossoverChannelShortEngine` from snapshots."""

    def __init__(self, config: MaCrossoverChannelShortConfig) -> None:
        self._config = config
        self._engine = MaCrossoverChannelShortEngine(config)
