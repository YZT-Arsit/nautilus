"""Ghost Trader long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`GhostTraderLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.ghost_trader_long.engine import HOLD, GhostTraderLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "ght_l_bar_open"
_HIGH = "ght_l_bar_high"
_LOW = "ght_l_bar_low"
_CLOSE = "ght_l_bar_close"
_VOLUME = "ght_l_bar_volume"


class GhostTraderLongStrategy:
    """Adapter: drive :class:`GhostTraderLongEngine` from snapshots."""

    def __init__(self, config: GhostTraderLongConfig) -> None:
        self._config = config
        self._engine = GhostTraderLongEngine(config)
