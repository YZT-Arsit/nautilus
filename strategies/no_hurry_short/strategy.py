"""No Hurry short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`NoHurryShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.no_hurry_short.engine import HOLD, NoHurryShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "nohurry_s_bar_open"
_HIGH = "nohurry_s_bar_high"
_LOW = "nohurry_s_bar_low"
_CLOSE = "nohurry_s_bar_close"
_VOLUME = "nohurry_s_bar_volume"


class NoHurryShortStrategy:
    """Adapter: drive :class:`NoHurryShortEngine` from snapshots."""

    def __init__(self, config: NoHurryShortConfig) -> None:
        self._config = config
        self._engine = NoHurryShortEngine(config)
