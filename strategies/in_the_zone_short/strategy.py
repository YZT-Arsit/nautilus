"""In The Zone short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`InTheZoneShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.in_the_zone_short.engine import HOLD, InTheZoneShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "itz_s_bar_open"
_HIGH = "itz_s_bar_high"
_LOW = "itz_s_bar_low"
_CLOSE = "itz_s_bar_close"
_VOLUME = "itz_s_bar_volume"


class InTheZoneShortStrategy:
    """Adapter: drive :class:`InTheZoneShortEngine` from snapshots."""

    def __init__(self, config: InTheZoneShortConfig) -> None:
        self._config = config
        self._engine = InTheZoneShortEngine(config)
