"""Self-contained event/result types for the VWM short strategy.

These were previously imported from ``nautilus_ext.strategies.*``. They are
copied here verbatim (only the fields VWM uses) so the strategy no longer
depends on the removed ``nautilus_ext`` layer. Pure Python, no dependencies.
"""
