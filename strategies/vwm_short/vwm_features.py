...
Ported verbatim from ``feature_engine.vwm_features``; now depends only on the
local ``indicators`` and ``signal_types`` modules.

VWM = XAverage(Vol * Momentum(Close, MomLen), AvgLen)
AATR = AvgTrueRange(ATRLen)
bull_setup = CrossOver(VWM, 0) ; bear_setup = CrossUnder(VWM, 0)
"""
...
from strategies.vwm_short.indicators import (
    AtrFeature,
    EmaFeature,
    RawMomentumFeature,
    cross_over,
