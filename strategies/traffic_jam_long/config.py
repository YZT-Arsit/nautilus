"""Traffic Jam long strategy configuration (user-facing parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``Traffic_Jam_L`` system — the long-side
mirror of ``Traffic_Jam_S``. Uses the ADX (from Wilder's DMI) to detect a ranging
market, then fades ``consec_bars`` consecutive down-closes with an ATR protective
...
class TrafficJamLongConfig:
    """User-facing parameters for the Traffic Jam long strategy."""
    ...
    consec_bars: int = 3                   # ConsecBars: consecutive down-closes to fade
