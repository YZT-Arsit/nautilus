"""Displaced-Bollinger long — pure decision engine (position-aware, offline).
...
Ported from the TradeBlazer ``DisplacedBoll_L`` system (long mirror of
``DisplacedBoll_S``):
...
* the channel mid-line is **displaced back** ``Disp`` bars while the width is
  ...
  SDmult``;
* entry (long): flat and ``High >= DispTop[1]`` -> long at ``Max(Open,
  DispTop[1])``;
* exit (sell), once ``BarsSinceEntry > 0``: ``Low <= DispBottom[1]`` -> sell at
  ``Min(Open, DispBottom[1])``.

Faithful TradeBlazer semantics preserved: the entry/exit read the **previous-bar**
band levels ``DispTop[1]`` / ``DispBottom[1]`` (snapshotted before the roll), while
the trigger uses the current bar's ``High`` / ``Low`` / ``Open``; the mid-line uses
...
class DisplacedBollLongEngine:
    """Pure, position-aware Displaced-Bollinger long engine."""

    def __init__(self, config: DisplacedBollLongConfig) -> None:
        self.cfg = config
    ...
    def update(self, open_: float, high: float, low: float, close: float, volume: float):
        ...
        # 2. ENTRY (open long): break of the previous upper band.
        if (
            not acted and mp_start == 0
            and self._prev_top is not None and high >= self._prev_top
        ):
            entry_price = max(open_, self._prev_top)
            self.position = 1
            self.bars_since_entry = 0
            self.entry_price = entry_price
            signal, reason, acted = BUY, "enter_long", True

        # 3. EXIT (sell): break of the previous lower band.
        if (
