"""Displaced-Bollinger long strategy configuration (parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``DisplacedBoll_L`` system — a breakout of a
Bollinger channel whose **mid-line is displaced back** ``Disp`` bars while the band
width uses the current standard deviation. Long-only: a break of the (previous)
upper band buys; the position sells on a break of the (previous) lower band.
"""
...
class DisplacedBollLongConfig:
    """User-facing parameters for the Displaced-Bollinger long strategy."""
