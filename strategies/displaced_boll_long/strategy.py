"""Displaced-Bollinger long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`DisplacedBollLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.displaced_boll_long.engine import HOLD, DisplacedBollLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "dboll_l_bar_open"
_HIGH = "dboll_l_bar_high"
_LOW = "dboll_l_bar_low"
_CLOSE = "dboll_l_bar_close"
_VOLUME = "dboll_l_bar_volume"


class DisplacedBollLongStrategy:
    """Adapter: drive :class:`DisplacedBollLongEngine` from snapshots."""

    def __init__(self, config: DisplacedBollLongConfig) -> None:
        self._config = config
        self._engine = DisplacedBollLongEngine(config)
