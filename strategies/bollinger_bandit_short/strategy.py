"""Bollinger Bandit short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`BollingerBanditShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.bollinger_bandit_short.engine import HOLD, BollingerBanditShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "bbandit_s_bar_open"
_HIGH = "bbandit_s_bar_high"
_LOW = "bbandit_s_bar_low"
_CLOSE = "bbandit_s_bar_close"
_VOLUME = "bbandit_s_bar_volume"


class BollingerBanditShortStrategy:
    """Adapter: drive :class:`BollingerBanditShortEngine` from snapshots."""

    def __init__(self, config: BollingerBanditShortConfig) -> None:
        self._config = config
        self._engine = BollingerBanditShortEngine(config)
