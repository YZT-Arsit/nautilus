"""Open/Close Histogram long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`OpenCloseHistogramLongEngine`. Reads raw open/high/low/close/volume off
...
from strategies.open_close_histogram_long.engine import HOLD, OpenCloseHistogramLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "ochisto_l_bar_open"
_HIGH = "ochisto_l_bar_high"
_LOW = "ochisto_l_bar_low"
_CLOSE = "ochisto_l_bar_close"
_VOLUME = "ochisto_l_bar_volume"


class OpenCloseHistogramLongStrategy:
    """Adapter: drive :class:`OpenCloseHistogramLongEngine` from snapshots."""

    def __init__(self, config: OpenCloseHistogramLongConfig) -> None:
        self._config = config
        self._engine = OpenCloseHistogramLongEngine(config)
