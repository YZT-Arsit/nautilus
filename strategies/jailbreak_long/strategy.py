"""JailBreak long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`JailBreakLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.jailbreak_long.engine import HOLD, JailBreakLongEngine
...
_HIGH = "jb_l_bar_high"
_LOW = "jb_l_bar_low"
_CLOSE = "jb_l_bar_close"
_VOLUME = "jb_l_bar_volume"


class JailBreakLongStrategy:
    """Adapter: drive :class:`JailBreakLongEngine` from snapshots."""

    def __init__(self, config: JailBreakLongConfig) -> None:
        self._config = config
        self._engine = JailBreakLongEngine(config)
