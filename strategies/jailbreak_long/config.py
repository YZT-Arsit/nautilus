"""JailBreak long strategy configuration (parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``JailBreakSys_L`` system — the long mirror
of ``jailbreak_short``: a break above the long-period (``max(Length1, Length2)``)
high channel opens a long, exited by an ATR protective stop or a break below the
short-period (``min(Length1, Length2)``) low channel.
"""
...
class JailBreakLongConfig:
    """User-facing parameters for the JailBreak long strategy."""
