"""RedRover short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`RedRoverShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.redrover_short.engine import HOLD, RedRoverShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "redrover_s_bar_open"
_HIGH = "redrover_s_bar_high"
_LOW = "redrover_s_bar_low"
_CLOSE = "redrover_s_bar_close"
_VOLUME = "redrover_s_bar_volume"


class RedRoverShortStrategy:
    """Adapter: drive :class:`RedRoverShortEngine` from snapshots."""

    def __init__(self, config: RedRoverShortConfig) -> None:
        self._config = config
        self._engine = RedRoverShortEngine(config)
