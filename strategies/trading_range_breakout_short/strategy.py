"""Trading Range Breakout short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`TradingRangeBreakoutShortEngine`. Reads raw open/high/low/close/volume
...
from strategies.trading_range_breakout_short.engine import HOLD, TradingRangeBreakoutShortEngine
...
class TradingRangeBreakoutShortStrategy:
    """Adapter: drive :class:`TradingRangeBreakoutShortEngine` from snapshots."""

    def __init__(self, config: TradingRangeBreakoutShortConfig) -> None:
        self._config = config
        self._engine = TradingRangeBreakoutShortEngine(config)
