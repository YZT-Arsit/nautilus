"""Open/Close Histogram short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`OpenCloseHistogramShortEngine`. Reads raw open/high/low/close/volume off
...
from strategies.open_close_histogram_short.engine import HOLD, OpenCloseHistogramShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "ochisto_s_bar_open"
_HIGH = "ochisto_s_bar_high"
_LOW = "ochisto_s_bar_low"
_CLOSE = "ochisto_s_bar_close"
_VOLUME = "ochisto_s_bar_volume"


class OpenCloseHistogramShortStrategy:
    """Adapter: drive :class:`OpenCloseHistogramShortEngine` from snapshots."""

    def __init__(self, config: OpenCloseHistogramShortConfig) -> None:
        self._config = config
        self._engine = OpenCloseHistogramShortEngine(config)
