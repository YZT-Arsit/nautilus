"""MA Support/Resistance long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`MaSupResLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.ma_sup_res_long.engine import HOLD, MaSupResLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "masr_l_bar_open"
_HIGH = "masr_l_bar_high"
_LOW = "masr_l_bar_low"
_CLOSE = "masr_l_bar_close"
_VOLUME = "masr_l_bar_volume"


class MaSupResLongStrategy:
    """Adapter: drive :class:`MaSupResLongEngine` from snapshots."""

    def __init__(self, config: MaSupResLongConfig) -> None:
        self._config = config
        self._engine = MaSupResLongEngine(config)
