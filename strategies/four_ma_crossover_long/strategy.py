"""Four-MA Crossover long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`FourMaCrossoverLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.four_ma_crossover_long.engine import HOLD, FourMaCrossoverLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "fmac_l_bar_open"
_HIGH = "fmac_l_bar_high"
_LOW = "fmac_l_bar_low"
_CLOSE = "fmac_l_bar_close"
_VOLUME = "fmac_l_bar_volume"


class FourMaCrossoverLongStrategy:
    """Adapter: drive :class:`FourMaCrossoverLongEngine` from snapshots."""

    def __init__(self, config: FourMaCrossoverLongConfig) -> None:
        self._config = config
        self._engine = FourMaCrossoverLongEngine(config)
