"""King Keltner short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`KingKeltnerShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.king_keltner_short.engine import HOLD, KingKeltnerShortEngine
...
_HIGH = "kk_s_bar_high"
_LOW = "kk_s_bar_low"
_CLOSE = "kk_s_bar_close"
_VOLUME = "kk_s_bar_volume"


class KingKeltnerShortStrategy:
    """Adapter: drive :class:`KingKeltnerShortEngine` from snapshots."""

    def __init__(self, config: KingKeltnerShortConfig) -> None:
        self._config = config
        self._engine = KingKeltnerShortEngine(config)
