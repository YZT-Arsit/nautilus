"""Keltner Channel long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`KeltnerChannelLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.keltner_channel_long.engine import HOLD, KeltnerChannelLongEngine
...
_HIGH = "kc_l_bar_high"
_LOW = "kc_l_bar_low"
_CLOSE = "kc_l_bar_close"
_VOLUME = "kc_l_bar_volume"


class KeltnerChannelLongStrategy:
    """Adapter: drive :class:`KeltnerChannelLongEngine` from snapshots."""

    def __init__(self, config: KeltnerChannelLongConfig) -> None:
        self._config = config
        self._engine = KeltnerChannelLongEngine(config)
