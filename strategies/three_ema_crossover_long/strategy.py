"""Three EMA Crossover long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`ThreeEmaCrossoverLongEngine`. Reads raw open/high/low/close/volume off
...
from strategies.three_ema_crossover_long.engine import HOLD, ThreeEmaCrossoverLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "three_ema_l_bar_open"
_HIGH = "three_ema_l_bar_high"
_LOW = "three_ema_l_bar_low"
_CLOSE = "three_ema_l_bar_close"
_VOLUME = "three_ema_l_bar_volume"


class ThreeEmaCrossoverLongStrategy:
    """Adapter: drive :class:`ThreeEmaCrossoverLongEngine` from snapshots."""

    def __init__(self, config: ThreeEmaCrossoverLongConfig) -> None:
        self._config = config
        self._engine = ThreeEmaCrossoverLongEngine(config)
