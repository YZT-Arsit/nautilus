"""ADX + MA-channel long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`AdxMaChannelLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.adx_ma_channel_long.engine import HOLD, AdxMaChannelLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "adxmac_l_bar_open"
_HIGH = "adxmac_l_bar_high"
_LOW = "adxmac_l_bar_low"
_CLOSE = "adxmac_l_bar_close"
_VOLUME = "adxmac_l_bar_volume"


class AdxMaChannelLongStrategy:
    """Adapter: drive :class:`AdxMaChannelLongEngine` from snapshots."""

    def __init__(self, config: AdxMaChannelLongConfig) -> None:
        self._config = config
        self._engine = AdxMaChannelLongEngine(config)
