"""Ghost Trader short — pure decision engine (position-aware, offline-testable).
...
Ported from the TradeBlazer ``GhostTrader_S`` system:
...
* ``AvgValue1 = XAverage(Close, FastLength)``; ``AvgValue2 = XAverage(Close,
  ...
  the 20-bar Donchian channel ``ExitHiBand = Highest(High, 20)``;
* a **simulated** short is tracked continuously via ``myPosition`` /
  ``myEntryPrice`` / ``myExitPrice`` / ``myProfit``:
  - sim entry (short): ``myPosition == 0`` and ``myPosition[1] == 0``,
    ``AvgValue1[1] < AvgValue2[1]``, ``RSIValue[1] > OverSold`` and ``Low <=
    Low[1]`` -> ``myEntryPrice = Min(Open, Low[1])``, ``myPosition = -1``;
  - sim exit: ``myPosition == -1`` and ``High >= ExitHiBand[1]`` -> ``myExitPrice =
    Max(Open, ExitHiBand[1])``, ``myProfit = myEntryPrice - myExitPrice``,
    ``myPosition = 0``;
* a **real** order is placed only in step with the simulated one AND gated on the
  prior simulated result: the real short fires at sim entry **only if** ``myProfit
  < 0`` (the last simulated trade lost); the real cover fires at sim exit whenever
  a real short is open.

Faithful TradeBlazer semantics preserved: the setup reads ``AvgValue1[1]`` /
``AvgValue2[1]`` / ``RSIValue[1]`` / ``ExitHiBand[1]`` / ``Low[1]`` /
...
Real entry and cover can never fire on one bar (the sim setups are mutually
...
class GhostTraderShortEngine:
    """Pure, position-aware Ghost Trader short engine."""
    ...
    def __init__(self, config: GhostTraderShortConfig) -> None:
        ...
        self._prev_exit_hi: float | None = None
    ...
    def update(self, open_: float, high: float, low: float, close: float, volume: float):
        ...
        exit_hi = max(self._highs)

        prev_my_position = self._prev_my_position
        signal, reason = HOLD, "hold"
        acted = False

        # 4. SIM EXIT (+ real cover): break above the prior Donchian upper channel.
        if (
            prev_my_position == -1 and self._prev_exit_hi is not None
            and high >= self._prev_exit_hi
        ):
            self.my_exit_price = max(open_, self._prev_exit_hi)
            self.my_profit = self.my_entry_price - self.my_exit_price
            self.my_position = 0
            if self.position == -1:
                self.position = 0
                self.bars_since_entry = 0
        ...
        # 5. SIM ENTRY (+ conditional real short): trend-down + RSI + new low.
        if (
            self.my_position == 0 and prev_my_position == 0
            and self._prev_avg1 is not None and self._prev_avg2 is not None
            and self._prev_rsi is not None and self._prev_low is not None
            and self._prev_avg1 < self._prev_avg2 and self._prev_rsi > cfg.over_sold
            and low <= self._prev_low
        ):
            self.my_entry_price = min(open_, self._prev_low)
            self.my_position = -1
            # Real short only after a losing simulated trade.
