"""JailBreak short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`JailBreakShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.jailbreak_short.engine import HOLD, JailBreakShortEngine
...
_HIGH = "jb_s_bar_high"
_LOW = "jb_s_bar_low"
_CLOSE = "jb_s_bar_close"
_VOLUME = "jb_s_bar_volume"


class JailBreakShortStrategy:
    """Adapter: drive :class:`JailBreakShortEngine` from snapshots."""

    def __init__(self, config: JailBreakShortConfig) -> None:
        self._config = config
        self._engine = JailBreakShortEngine(config)
