"""Three EMA Crossover short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`ThreeEmaCrossoverShortEngine`. Reads raw open/high/low/close/volume off
...
from strategies.three_ema_crossover_short.engine import HOLD, ThreeEmaCrossoverShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "three_ema_s_bar_open"
_HIGH = "three_ema_s_bar_high"
_LOW = "three_ema_s_bar_low"
_CLOSE = "three_ema_s_bar_close"
_VOLUME = "three_ema_s_bar_volume"


class ThreeEmaCrossoverShortStrategy:
    """Adapter: drive :class:`ThreeEmaCrossoverShortEngine` from snapshots."""

    def __init__(self, config: ThreeEmaCrossoverShortConfig) -> None:
        self._config = config
        self._engine = ThreeEmaCrossoverShortEngine(config)
