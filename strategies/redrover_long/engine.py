"""RedRover long — pure decision engine (position-aware, offline-testable).

Long-side mirror of ``strategies/redrover_short/engine.py``. Holds **only** the
...
Ported from the TradeBlazer ``RedRover_L`` system:
...
* entry (long): flat, ``High >= Resistance[1] + tick`` (breaks the prior
  resistance), ``Vol > 0`` -> long at ``Max(Open, Resistance[1] + tick)``; at the
  entry bar record ``myExitPrice = EntryPrice + ATR * atr_s`` (this bar's ATR);
* exit (sell), once ``BarsSinceEntry > 0``: profit target ``High >= myExitPrice``
  -> ``Max(Open, myExitPrice)``; else reverse ``Low <= Support[1] - tick`` ->
  ``Min(Open, Support[1] - tick)``.

Faithful TradeBlazer semantics preserved (identical to the short engine, mirrored
to the long side): the entry / reverse compare against the **previous** bar's
Resistance / Support (``[1]``); ``myExitPrice`` is fixed at the entry bar from that
bar's ATR and the entry fill, and held; ``MarketPosition == 0`` / ``== 1`` uses the
bar-start position and the exit is gated by ``BarsSinceEntry > 0``. Exit priority:
profit target then reverse. Simple-mean ATR (documented deviation from the Wilder
builtin).
"""
...
class RedRoverLongEngine:
    """Pure, position-aware RedRover long engine."""

    def __init__(self, config: RedRoverLongConfig) -> None:
        self.cfg = config
    ...
    def update(self, open_: float, high: float, low: float, close: float, volume: float):
        ...
        # 3. ENTRY (open long): break the prior resistance line.
        if (
            not acted and mp_start == 0
            and self._prev_resistance is not None and atr is not None
            and high >= self._prev_resistance + cfg.tick
            and volume > 0
        ):
            entry_price = max(open_, self._prev_resistance + cfg.tick)
            self.position = 1
            self.bars_since_entry = 0
        ...
        # 4. EXIT (sell): profit target -> reverse break.
        if not acted and mp_start == 1 and self.bars_since_entry > 0 and volume > 0:
            if self.my_exit_price is not None and high >= self.my_exit_price:
                self._flat()
                signal, reason, acted = SELL, "exit_take_profit", True
            elif (
                self._prev_support is not None
