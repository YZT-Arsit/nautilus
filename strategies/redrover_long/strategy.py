"""RedRover long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`RedRoverLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.redrover_long.engine import HOLD, RedRoverLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "redrover_l_bar_open"
_HIGH = "redrover_l_bar_high"
_LOW = "redrover_l_bar_low"
_CLOSE = "redrover_l_bar_close"
_VOLUME = "redrover_l_bar_volume"


class RedRoverLongStrategy:
    """Adapter: drive :class:`RedRoverLongEngine` from snapshots."""

    def __init__(self, config: RedRoverLongConfig) -> None:
        self._config = config
        self._engine = RedRoverLongEngine(config)
