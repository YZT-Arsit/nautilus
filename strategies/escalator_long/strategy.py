"""Escalator long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`EscalatorLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.escalator_long.engine import HOLD, EscalatorLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "esc_l_bar_open"
_HIGH = "esc_l_bar_high"
_LOW = "esc_l_bar_low"
_CLOSE = "esc_l_bar_close"
_VOLUME = "esc_l_bar_volume"


class EscalatorLongStrategy:
    """Adapter: drive :class:`EscalatorLongEngine` from snapshots."""

    def __init__(self, config: EscalatorLongConfig) -> None:
        self._config = config
        self._engine = EscalatorLongEngine(config)
