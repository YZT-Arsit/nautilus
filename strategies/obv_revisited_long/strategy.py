"""OBV Revisited long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`ObvRevisitedLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.obv_revisited_long.engine import HOLD, ObvRevisitedLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "obv_l_bar_open"
_HIGH = "obv_l_bar_high"
_LOW = "obv_l_bar_low"
_CLOSE = "obv_l_bar_close"
_VOLUME = "obv_l_bar_volume"


class ObvRevisitedLongStrategy:
    """Adapter: drive :class:`ObvRevisitedLongEngine` from snapshots."""

    def __init__(self, config: ObvRevisitedLongConfig) -> None:
        self._config = config
        self._engine = ObvRevisitedLongEngine(config)
