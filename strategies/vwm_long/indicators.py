"""Self-contained indicator primitives for the VWM long strategy.

Copied verbatim from ``strategies/vwm_short/indicators.py``. ``EmaFeature`` /
``AtrFeature`` wrap Nautilus native indicators (``ExponentialMovingAverage`` /
``AverageTrueRange``) — this preserves the exact XAverage / AvgTrueRange
semantics of the original TradeBlazer port. ``nautilus_trader`` is imported at
module top; this module is only imported lazily (when a VWM strategy instance
is built).
"""
