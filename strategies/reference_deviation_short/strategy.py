"""Reference Deviation System short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`ReferenceDeviationShortEngine`. Reads raw open/high/low/close/volume off
...
from strategies.reference_deviation_short.engine import HOLD, ReferenceDeviationShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "refdev_s_bar_open"
_HIGH = "refdev_s_bar_high"
_LOW = "refdev_s_bar_low"
_CLOSE = "refdev_s_bar_close"
_VOLUME = "refdev_s_bar_volume"


class ReferenceDeviationShortStrategy:
    """Adapter: drive :class:`ReferenceDeviationShortEngine` from snapshots."""

    def __init__(self, config: ReferenceDeviationShortConfig) -> None:
        self._config = config
        self._engine = ReferenceDeviationShortEngine(config)
    ...
    def on_snapshot(self, snapshot: FeatureSnapshot) -> str:
        """Legacy baseline entry point using the immediate-fill wrapper."""
