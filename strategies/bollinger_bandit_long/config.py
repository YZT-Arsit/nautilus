"""Bollinger Bandit long strategy configuration (parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``BollingerBandit_L`` system — a Bollinger
upper-band breakout gated by a rate-of-change filter, exited by an adaptive-length
moving average whose period shrinks the longer the position is held. Long-only.
"""
...
class BollingerBanditLongConfig:
    """User-facing parameters for the Bollinger Bandit long strategy."""

    bollinger_lengths: int = 50   # bollingerLengths: mid-line SMA / std period
    offset: float = 1.25          # Offset: upper-band standard-deviation multiple
