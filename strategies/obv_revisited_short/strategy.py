"""OBV Revisited short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`ObvRevisitedShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.obv_revisited_short.engine import HOLD, ObvRevisitedShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "obv_s_bar_open"
_HIGH = "obv_s_bar_high"
_LOW = "obv_s_bar_low"
_CLOSE = "obv_s_bar_close"
_VOLUME = "obv_s_bar_volume"


class ObvRevisitedShortStrategy:
    """Adapter: drive :class:`ObvRevisitedShortEngine` from snapshots."""

    def __init__(self, config: ObvRevisitedShortConfig) -> None:
        self._config = config
        self._engine = ObvRevisitedShortEngine(config)
