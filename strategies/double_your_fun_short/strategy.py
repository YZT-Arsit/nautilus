"""DoubleYourFun short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`DoubleYourFunShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.double_your_fun_short.engine import HOLD, DoubleYourFunShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "dyf_s_bar_open"
_HIGH = "dyf_s_bar_high"
_LOW = "dyf_s_bar_low"
_CLOSE = "dyf_s_bar_close"
_VOLUME = "dyf_s_bar_volume"


class DoubleYourFunShortStrategy:
    """Adapter: drive :class:`DoubleYourFunShortEngine` from snapshots."""

    def __init__(self, config: DoubleYourFunShortConfig) -> None:
        self._config = config
        self._engine = DoubleYourFunShortEngine(config)
