"""Superman System short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`SupermanShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.superman_short.engine import HOLD, SupermanShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "superman_s_bar_open"
_HIGH = "superman_s_bar_high"
_LOW = "superman_s_bar_low"
_CLOSE = "superman_s_bar_close"
_VOLUME = "superman_s_bar_volume"


class SupermanShortStrategy:
    """Adapter: drive :class:`SupermanShortEngine` from snapshots."""

    def __init__(self, config: SupermanShortConfig) -> None:
        self._config = config
        self._engine = SupermanShortEngine(config)
