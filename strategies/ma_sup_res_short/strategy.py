"""MA Support/Resistance short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`MaSupResShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.ma_sup_res_short.engine import HOLD, MaSupResShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "masr_s_bar_open"
_HIGH = "masr_s_bar_high"
_LOW = "masr_s_bar_low"
_CLOSE = "masr_s_bar_close"
_VOLUME = "masr_s_bar_volume"


class MaSupResShortStrategy:
    """Adapter: drive :class:`MaSupResShortEngine` from snapshots."""

    def __init__(self, config: MaSupResShortConfig) -> None:
        self._config = config
        self._engine = MaSupResShortEngine(config)
