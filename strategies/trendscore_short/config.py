"""TrendScore short strategy configuration (user-facing parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``TrendScore_S`` system — a short-only
trend system that scores the close against the prior ``look_back`` closes and
trades when both price and score sit below their moving averages, with an
ATR protective / trailing / break-even stop stack.
"""
...
class TrendScoreShortConfig:
    """User-facing parameters for the TrendScore short strategy."""
