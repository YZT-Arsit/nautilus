"""TrendScore short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`TrendScoreShortEngine`. It reads raw open/high/low/close/volume off the
snapshot (exposed as identity-passthrough features, see ``plugin.build_specs``),
drives the engine, and records the latest decision reason. Imports **no**
``nautilus_trader``.
...
from strategies.trendscore_short.engine import HOLD, TrendScoreShortEngine
...
class TrendScoreShortStrategy:
    """Adapter: drive :class:`TrendScoreShortEngine` from feature snapshots."""

    def __init__(self, config: TrendScoreShortConfig) -> None:
        self._config = config
        self._engine = TrendScoreShortEngine(config)
