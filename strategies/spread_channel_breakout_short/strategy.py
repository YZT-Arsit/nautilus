"""Spread Channel Breakout short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`SpreadChannelBreakoutShortEngine`. Reads raw open/high/low/close/volume
off the snapshot (identity-passthrough features, see ``plugin.build_specs``),
drives the engine, records the latest reason. Imports **no** ``nautilus_trader``.

The engine treats the snapshot's open/close as the spread bar's open/close (see
its module docstring for the single-series vs two-leg note). Signals are
``BUY``/``SELL``/``HOLD``; the signal->order decision stays in
...
from strategies.spread_channel_breakout_short.engine import HOLD, SpreadChannelBreakoutShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "spread_cb_s_bar_open"
_HIGH = "spread_cb_s_bar_high"
_LOW = "spread_cb_s_bar_low"
_CLOSE = "spread_cb_s_bar_close"
_VOLUME = "spread_cb_s_bar_volume"


class SpreadChannelBreakoutShortStrategy:
    """Adapter: drive :class:`SpreadChannelBreakoutShortEngine` from snapshots."""

    def __init__(self, config: SpreadChannelBreakoutShortConfig) -> None:
        self._config = config
        self._engine = SpreadChannelBreakoutShortEngine(config)
