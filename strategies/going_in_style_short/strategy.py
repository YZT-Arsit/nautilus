"""Going in Style short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`GoingInStyleShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.going_in_style_short.engine import HOLD, GoingInStyleShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "gis_s_bar_open"
_HIGH = "gis_s_bar_high"
_LOW = "gis_s_bar_low"
_CLOSE = "gis_s_bar_close"
_VOLUME = "gis_s_bar_volume"


class GoingInStyleShortStrategy:
    """Adapter: drive :class:`GoingInStyleShortEngine` from snapshots."""

    def __init__(self, config: GoingInStyleShortConfig) -> None:
        self._config = config
        self._engine = GoingInStyleShortEngine(config)
