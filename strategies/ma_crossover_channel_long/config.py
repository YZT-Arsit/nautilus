"""MA-Crossover Channel-Breakout long strategy configuration (parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``MovingAverageCrossOver_L`` system — a
fast/slow MA crossover that arms a channel-breakout entry, with a trend-reversal
exit, a periodic-low trailing stop, and a post-trailing-stop re-entry. Registered
as ``ma_crossover_channel_long`` to avoid colliding with the simpler
...
class MaCrossoverChannelLongConfig:
    """User-facing parameters for the MA-Crossover Channel-Breakout long strategy."""
    ...
    trail_bar: int = 8             # TrailBar: lowest-low lookback for the trailing stop
