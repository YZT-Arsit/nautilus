"""Going in Style long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`GoingInStyleLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.going_in_style_long.engine import HOLD, GoingInStyleLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "gis_l_bar_open"
_HIGH = "gis_l_bar_high"
_LOW = "gis_l_bar_low"
_CLOSE = "gis_l_bar_close"
_VOLUME = "gis_l_bar_volume"


class GoingInStyleLongStrategy:
    """Adapter: drive :class:`GoingInStyleLongEngine` from snapshots."""

    def __init__(self, config: GoingInStyleLongConfig) -> None:
        self._config = config
        self._engine = GoingInStyleLongEngine(config)
