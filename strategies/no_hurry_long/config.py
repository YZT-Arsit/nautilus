"""No Hurry long strategy configuration (parameters only).

Pure dataclass: no Nautilus, no pandas, no feature_engine / strategy_framework
imports. Ported from the TradeBlazer ``NoHurrySystem_L`` system — the long mirror
of ``no_hurry_short``: a *shifted* (time-delayed) high/low channel breakout with
an ATR trailing stop. A break of the upper channel value from ``chan_delay + 1``
bars ago opens the long; a decline back through either the trailing ATR stop or
the shifted lower channel flattens it.
"""
...
class NoHurryLongConfig:
    """User-facing parameters for the No Hurry long strategy."""
