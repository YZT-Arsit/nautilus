"""No Hurry long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`NoHurryLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.no_hurry_long.engine import HOLD, NoHurryLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "nohurry_l_bar_open"
_HIGH = "nohurry_l_bar_high"
_LOW = "nohurry_l_bar_low"
_CLOSE = "nohurry_l_bar_close"
_VOLUME = "nohurry_l_bar_volume"


class NoHurryLongStrategy:
    """Adapter: drive :class:`NoHurryLongEngine` from snapshots."""

    def __init__(self, config: NoHurryLongConfig) -> None:
        self._config = config
        self._engine = NoHurryLongEngine(config)
