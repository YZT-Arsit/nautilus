"""Dynamic Breakout II short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`DynamicBreakoutShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.dynamic_breakout_short.engine import HOLD, DynamicBreakoutShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "dbo_s_bar_open"
_HIGH = "dbo_s_bar_high"
_LOW = "dbo_s_bar_low"
_CLOSE = "dbo_s_bar_close"
_VOLUME = "dbo_s_bar_volume"


class DynamicBreakoutShortStrategy:
    """Adapter: drive :class:`DynamicBreakoutShortEngine` from snapshots."""

    def __init__(self, config: DynamicBreakoutShortConfig) -> None:
        self._config = config
        self._engine = DynamicBreakoutShortEngine(config)
