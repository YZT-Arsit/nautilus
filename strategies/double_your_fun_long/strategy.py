"""DoubleYourFun long strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`DoubleYourFunLongEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.double_your_fun_long.engine import HOLD, DoubleYourFunLongEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "dyf_l_bar_open"
_HIGH = "dyf_l_bar_high"
_LOW = "dyf_l_bar_low"
_CLOSE = "dyf_l_bar_close"
_VOLUME = "dyf_l_bar_volume"


class DoubleYourFunLongStrategy:
    """Adapter: drive :class:`DoubleYourFunLongEngine` from snapshots."""

    def __init__(self, config: DoubleYourFunLongConfig) -> None:
        self._config = config
        self._engine = DoubleYourFunLongEngine(config)
