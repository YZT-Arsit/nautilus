"""First-PullBack short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`FirstPullbackShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.first_pullback_short.engine import HOLD, FirstPullbackShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "fpb_s_bar_open"
_HIGH = "fpb_s_bar_high"
_LOW = "fpb_s_bar_low"
_CLOSE = "fpb_s_bar_close"
_VOLUME = "fpb_s_bar_volume"


class FirstPullbackShortStrategy:
    """Adapter: drive :class:`FirstPullbackShortEngine` from snapshots."""

    def __init__(self, config: FirstPullbackShortConfig) -> None:
        self._config = config
        self._engine = FirstPullbackShortEngine(config)
