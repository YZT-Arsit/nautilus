"""Escalator short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`EscalatorShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.escalator_short.engine import HOLD, EscalatorShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "esc_s_bar_open"
_HIGH = "esc_s_bar_high"
_LOW = "esc_s_bar_low"
_CLOSE = "esc_s_bar_close"
_VOLUME = "esc_s_bar_volume"


class EscalatorShortStrategy:
    """Adapter: drive :class:`EscalatorShortEngine` from snapshots."""

    def __init__(self, config: EscalatorShortConfig) -> None:
        self._config = config
        self._engine = EscalatorShortEngine(config)
