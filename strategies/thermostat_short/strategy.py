"""Thermostat short strategy adapter (FeatureSnapshot -> engine).

Thin glue between the runner's :class:`FeatureSnapshot` stream and the pure
:class:`ThermostatShortEngine`. Reads raw open/high/low/close/volume off the
...
from strategies.thermostat_short.engine import HOLD, ThermostatShortEngine

# Identity passthrough feature names (window=1 rolling mean == the raw field);
# shared with ``plugin.build_specs``.
_OPEN = "thermostat_s_bar_open"
_HIGH = "thermostat_s_bar_high"
_LOW = "thermostat_s_bar_low"
_CLOSE = "thermostat_s_bar_close"
_VOLUME = "thermostat_s_bar_volume"


class ThermostatShortStrategy:
    """Adapter: drive :class:`ThermostatShortEngine` from snapshots."""

    def __init__(self, config: ThermostatShortConfig) -> None:
        self._config = config
        self._engine = ThermostatShortEngine(config)
